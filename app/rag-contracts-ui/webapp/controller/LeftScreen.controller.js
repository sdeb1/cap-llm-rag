sap.ui.define([
    ...
    return Controller.extend("ragcontractsui.controller.LeftScreen", {
        ...
        onUploadFileBtnSelect: function(oEvent){
            ...
            this.fileUploadFragment ??= this.loadFragment({
                name: "ragcontractsui.view.FileUploading"
            });
        ...
        onManageFileBtnSelect: function(){
            ...
            this.fileManagementFragment ??= this.loadFragment({
                name: "ragcontractsui.view.FileManagement"
            });
        ...
        onGenerateVectorBtnClick: function(oEvent){
            ...
            this.requestEmbeddingGeneration(pdfFileID)
                .then((oReturn) => {
                    ...
                    MessageToast.show("Embeddings generation completed successfully for file.");
                })
                .catch((error) => {
                    const sleep = (ms) => {
                        const date = Date.now();
                    ...
                    if (error == 'Gateway Timeout') {
                        ...
                        while (count < 5) {
                            $.ajax({
                                url : sessionStorage.getItem("isDeployedVersion")==="true"?this.getBaseURL() + "/odata/v4/embedding-storage/checkEmbeddingCompletion":"/odata/v4/embedding-storage/checkEmbeddingCompletion",
                                type: "POST",
                        ...
                        if (success > 0) {
                            sleep(60000);
                            console.log("Embeddings generation completed successfully for file.")
                            MessageToast.show("Embeddings generation completed successfully for file.");
                        } else {
                            console.log("Embeddings not yet generated for file, please check after sometime.")
                            MessageToast.show("Embeddings not yet generated for file, please check after sometime.");
                        }
                        ...
                        this.byId("fileManagementFragment").setBusy(false);
                        console.log(error);
                        MessageToast.show("Embeddings generation failed for file, please try again.");
                    }                                                      
                });
        },
        ...
        onDeleteEmbedding: function(oEvent){
            ...
            const fileID = oAggregations.cells[1].getProperty("text");
            const fileName = oItem.getProperty("fileName");
            this.requestEmbeddingDelete(fileName)
            .then((result) => {
                this.byId("fileManagementFragment").setBusy(false);
                MessageToast.show(`All embeddings successfully deleted for selected file.`);			
            })
            .catch((error) => {
        ...
        requestEmbeddingDelete: function(fileName){
            const payload = JSON.stringify({
